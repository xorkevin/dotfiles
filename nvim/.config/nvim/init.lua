...
do
  ...
  local treesitter_hl_group = vim.api.nvim_create_augroup('k_treesitter_hl', { clear = true })
  vim.api.nvim_create_autocmd('FileType', {
    group = treesitter_hl_group,
    pattern = '*',
    callback = function(args)
      if vim.fn.getfsize(vim.api.nvim_buf_get_name(args.buf)) > highlight_file_size_limit then
        return
      end
